...
class BaselineRacer(object):
    def __init__(self, drone_name = "drone_1", plot_transform=True, viz_traj=True, viz_image_cv2=True):
        ...
        self.image_callback_thread = threading.Thread(target=self.repeat_timer_image_callback, args=(self.image_callback, 0.003))
        # self.odometry_callback_thread = threading.Thread(target=self.repeat_timer_odometry_callback, args=(self.odometry_callback, 0.002))
...
def isPassGate(baseline_racer, next_gate_pose, gate_passed_thresh):
    curr_position_linear = baseline_racer.airsim_client.simGetVehiclePose(vehicle_name = "drone_1").position

    dist_from_next_gate = math.sqrt( (curr_position_linear.x_val - next_gate_pose.x_val)**2
                                    + (curr_position_linear.y_val - next_gate_pose.y_val)**2
                                    + (curr_position_linear.z_val- next_gate_pose.z_val)**2)

    if(dist_from_next_gate < gate_passed_thresh):
        return True
    return False
