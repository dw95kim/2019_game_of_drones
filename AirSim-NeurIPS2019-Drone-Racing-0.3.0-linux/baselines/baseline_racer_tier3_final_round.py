...
MODEL_NAME = './object_detection/inference_graph_mt'
CWD_PATH = os.getcwd()
...
class BaselineRacer(object):
    ...
    def control_callback(self):
        ...
        if(self.isPassGate()):
            ...
            if(self.next_gate_idx == 10):
                ...
                drone_heading_vec = self.get_world_frame_vel_from_drone_frame_vel(self.airsim_client.simGetVehiclePose("drone_1").orientation, airsim.Vector3r(4, 2, 0))
                self.airsim_client.moveByVelocityAsync(drone_heading_vec.x_val, drone_heading_vec.y_val, drone_heading_vec.z_val, duration = 1).join()
            ...
            if(self.next_gate_idx == 16):
                ...
                drone_heading_vec = self.get_world_frame_vel_from_drone_frame_vel(self.airsim_client.simGetVehiclePose("drone_1").orientation, airsim.Vector3r(3, 0, 0))
                self.airsim_client.moveByVelocityAsync(drone_heading_vec.x_val, drone_heading_vec.y_val, drone_heading_vec.z_val, duration = 0.5).join()

            if(self.next_gate_idx == 17):
                print("pass gate next gate idx 17: go ahead")
                drone_heading_vec = self.get_world_frame_vel_from_drone_frame_vel(self.airsim_client.simGetVehiclePose("drone_1").orientation, airsim.Vector3r(8, 8, 2))
            ...
            if(self.next_gate_idx == 18):
                ...
                drone_heading_vec = self.get_world_frame_vel_from_drone_frame_vel(self.airsim_client.simGetVehiclePose("drone_1").orientation, airsim.Vector3r(4, -3, 0))
                self.airsim_client.moveByVelocityAsync(drone_heading_vec.x_val, drone_heading_vec.y_val, drone_heading_vec.z_val, duration = 1).join()
        ...
        if (self.detect_flag and (self.prev_vel_mag < 10 or self.next_gate_idx == 9 or self.next_gate_idx == 10)):
            with self.img_mutex:
                ...
                #############Control Gain param################
                Dist = 10
                velocity_x = velocity_x_o + (self.estimate_depth - Dist)*0.18 - abs(self.distance_y_curr) * 0.3  - abs(self.distance_z_curr) * 0.3 #0.35
                ...
                if(self.next_gate_idx == 21):
                    ...
                    velocity_vector_drone = airsim.Vector3r(cmd_vel_z, cmd_vel_y, cmd_vel_x)
                else:
            ...
            if(self.next_gate_idx > 17):
                target_lambda_pose = self.get_lam_point(gate_pose_array, drone_pose_array, 0.7)
            elif(self.next_gate_idx > 11):
                target_lambda_pose = self.get_lam_point(gate_pose_array, drone_pose_array, 0.56)            
            elif(self.next_gate_idx == 20):
                target_lambda_pose = self.get_lam_point(gate_pose_array, drone_pose_array, 0.25)
            else:
            ...
            if(self.next_gate_idx == 0):
                ...
                self.airsim_client.moveToPositionAsync(target_lambda_pose[0], target_lambda_pose[1], gate_pose_array[2] + 10, velocity = vel_fun)
            elif(2 < self.next_gate_idx < 7):
                self.airsim_client.moveToPositionAsync(target_lambda_pose[0], target_lambda_pose[1], gate_pose_array[2] + 10, velocity = vel_fun, yaw_mode=airsim.YawMode(True,angle))
            elif(self.next_gate_idx < 5):
                self.airsim_client.moveToPositionAsync(target_lambda_pose[0], target_lambda_pose[1], target_lambda_z_pose[2], velocity = vel_fun, yaw_mode=airsim.YawMode(True,angle))
                ...
                self.airsim_client.moveByVelocityAsync(start_drone_heading_vec.x_val, start_drone_heading_vec.y_val, start_drone_heading_vec.z_val, duration = 0.4).join()
            elif(self.next_gate_idx == 21):
                vel_error = airsim.Vector3r(target_lambda_pose[0], target_lambda_pose[1], target_lambda_z_pose[2] + 20) - drone_pose
                vel_mag = math.sqrt(vel_error.x_val ** 2 + vel_error.y_val ** 2 + vel_error.z_val ** 2)

                # vel_fun = 0.3 * vel_mag + (-0.084) * (abs(angle) / vel_mag) #0.543
                vel_fun = 8
                self.airsim_client.moveToPositionAsync(target_lambda_pose[0], target_lambda_pose[1], target_lambda_z_pose[2] + 10, velocity = vel_fun, yaw_mode=airsim.YawMode(True,angle))    
            else:
                if(self.go_ahead_flag == False):
                    if(self.next_gate_idx > 10):
                        self.airsim_client.moveToPositionAsync(target_lambda_pose[0], target_lambda_pose[1], target_lambda_z_pose[2], velocity = vel_fun*1.2, yaw_mode=airsim.YawMode(True,angle))
                    elif(self.next_gate_idx == 20):
                        self.airsim_client.moveToPositionAsync(target_lambda_pose[0], target_lambda_pose[1], target_lambda_z_pose[2] - 10, velocity = vel_fun*1.3, yaw_mode=airsim.YawMode(True,angle))
                    else:
                        self.airsim_client.moveToPositionAsync(target_lambda_pose[0], target_lambda_pose[1], target_lambda_z_pose[2], velocity = vel_fun, yaw_mode=airsim.YawMode(True,angle))
                else:
    ...
    def image_callback(self):
        ...
        #self.count += 1
        if(self.next_gate_idx == 21):
            request = [airsim.ImageRequest("bottom_cam", airsim.ImageType.Scene, False, False)]
            response = self.airsim_client_images.simGetImages(request)
            img_rgb_1d_b = np.fromstring(response[0].image_data_uint8, dtype=np.uint8)
            img_rgb_b = img_rgb_1d_b.reshape(response[0].height, response[0].width, 3)
