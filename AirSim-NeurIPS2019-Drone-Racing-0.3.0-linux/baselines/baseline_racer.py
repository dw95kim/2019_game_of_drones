...
class BaselineRacer(object):
    ...
    def fly_through_all_gates_one_by_one_with_moveOnSpline(self):
        ...
        return self.airsim_client.moveOnSplineAsync([self.gate_poses_ground_truth[0].position], vel_max=vel_max, acc_max=acc_max, 
    ...
    def image_callback(self):
        ...
        if self.viz_image_cv2:
            img_name = "./picture/" + str(self.count) + ".jpg"
            self.count += 1
...
def main(args):
    ...
    for i in range(len(baseline_racer.gate_poses_ground_truth)):
        while (1):
            ...
            drone_pose = baseline_racer.airsim_client.simGetVehiclePose("drone_1").position
            drone_ori = baseline_racer.airsim_client.simGetVehiclePose("drone_1").orientation

            gate_vec_in_drone = baseline_racer.get_drone_frame_vector_from_world_frame_vector(drone_ori, baseline_racer.get_gate_facing_vector_from_quaternion(gate_ori))
            ...
            baseline_racer.airsim_client.moveToPositionAsync(gate_pose_array[0], gate_pose_array[1], gate_pose_array[2], velocity = 1, yaw_mode=airsim.YawMode(True, angle))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--level_name', type=str, choices=["Soccer_Field_Easy", "Soccer_Field_Medium", "ZhangJiaJie_Medium", "Building99_Hard", 'Qualifier_Tier_1', 'Qualifier_Tier_2', 'Qualifier_Tier_3'], default="Qualifier_Tier_1")
    parser.add_argument('--planning_baseline_type', type=str, choices=["all_gates_at_once","all_gates_one_by_one"], default="all_gates_one_by_one")
