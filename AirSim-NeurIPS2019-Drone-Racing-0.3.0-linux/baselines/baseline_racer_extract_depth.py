...
class BaselineRacer(object):
    ...
    def gate_detection(self, img_rgb):
        with self.img_mutex:
            ...
            if N >= 1:  # in the case of more than one gates are detected, we want to select the nearest gate (biggest bounding box)
                ...
                #     print('----------------------------------')
                if Area_box <= 1.00 and Area_box >= 0.0:    # Feel free to change this number, set to 0 if don't want this effect
    ...
    def control_callback(self):
        gate_pose = self.gate_poses_ground_truth[0].position
        self.airsim_client.moveToPositionAsync(gate_pose.x_val, gate_pose.y_val, gate_pose.z_val, velocity = 1, vehicle_name = "drone_1").join()

        
        
        # Dist = 6
        # if(self.next_gate_idx == 5):
        #     velocity_x_o = 3
        # elif(self.next_gate_idx == 18):
        #     velocity_x_o = 1.6
        # elif(self.next_gate_idx == 20 or self.next_gate_idx == 19): 
        #     velocity_x_o = 1.6
        # elif(self.next_gate_idx == 13):
        #     velocity_x_o = 7
        #     Dist = 3
        # else:
        #     velocity_x_o = 5.5 # 3.8
        # duration = 0.05
        ...
        #         self.pass_position_vec = self.airsim_client.simGetVehiclePose(vehicle_name = "drone_1").position
        #         print(str(self.next_gate_idx) + " PASS")
        #     if (self.next_gate_idx == 10 and self.gate10_go_ahead == False):
        #         print("gate 10:  go ahead")
        ...
        #         velocity_x = velocity_x_o + (self.estimate_depth - Dist)*0.187 - abs(self.distance_y_curr) * 0.4 - abs(self.distance_z_curr) * 0.4
                
        #         Vely_P_Gain = velocity_x*0.62 # 0.47 is good when vel 3.8 #0.9 is good when vel = 3 #0.43
        #         Vely_D_Gain = velocity_x*0.13
        #         Velz_P_Gain = velocity_x*0.57 # 0.49 is good when vel 3.8 #0.9 is good when vel = 3 #0.43
        ...
        #     ## ground z val = 3.5
        #     if(target_lambda_z_pose[2] > 0.5):
        #         target_lambda_z_pose[2] = 0.5
            
        #     target_pose = gate_pose_array
        #     # if(target_pose[2] > 0.5):
        #     #     target_pose[2] = 0.5
        ...
        #     vel_error = airsim.Vector3r(target_pose[0], target_pose[1], target_pose[2]) - drone_pose
        ...
        #         self.airsim_client.moveToPositionAsync(target_lambda_pose[0], target_lambda_pose[1], target_lambda_z_pose[2], velocity = vel_fun, yaw_mode=airsim.YawMode(True,angle))
        #     elif(self.next_gate_idx == 13):
        #         vel_fun = 0.4 * vel_mag + (-0.2) * (abs(angle) / vel_mag) #0.543
        ...
        #         vel_fun = 0.45 * vel_mag + (-0.2) * (abs(angle) / vel_mag) #0.543
        #         self.airsim_client.moveToPositionAsync(target_lambda_pose[0], target_lambda_pose[1], target_pose[2] + 5, velocity = vel_fun, yaw_mode=airsim.YawMode(True,angle))
        #     elif(self.next_gate_idx == 19):
        #         vel_fun = 0.2 * vel_mag + (-0.2) * (abs(angle) / vel_mag) #0.543
        #         self.airsim_client.moveToPositionAsync(target_lambda_pose[0], target_lambda_pose[1], target_lambda_z_pose[2], velocity = vel_fun, yaw_mode=airsim.YawMode(True,angle))
        #     elif(self.next_gate_idx == 20):
        #         vel_fun = 0.1 * vel_mag + (-0.2) * (abs(angle) / vel_mag) #0.543
        ...
        #     else:
        #         vel_fun = 1
        #         self.airsim_client.moveToPositionAsync(target_pose[0], target_pose[1], target_pose[2], velocity = vel_fun, yaw_mode=airsim.YawMode(True,angle))
    ...
    def image_callback(self):
        ...
        path = "./picture_depth/picture_depth_" + str(self.day) + "_" + str(self.hour) + "_" + str(self.minute) + "/"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--level_name', type=str, choices=["Soccer_Field_Easy", "Soccer_Field_Medium", "ZhangJiaJie_Medium", "Building99_Hard", 'Final_Tier_1', 'Final_Tier_2', 'Final_Tier_3'], default="Soccer_Field_Easy")
